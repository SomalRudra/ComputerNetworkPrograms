...
void print_tcp_header(char* buff)
{
    ...
    memset(&source, 0, sizeof(source));
    source.sin_addr.s_addr = tcph->source;
...
void print_udp_header(char *buff){
    unsigned short iphdrlen;
...
int client_connect(int cli_port)
{
    ...
    //copy server details addres to cli_addr
