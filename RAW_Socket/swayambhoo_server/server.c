...
void CapitalizeAlternate(char* s)
{
	int i=0;
    while(i<strlen(s)){
        s[i]=toupper(s[i]);
...
void lowercaseAlternate(char* s)
{
    int i=0;
    while(i<strlen(s)){
        if(s[i]>='A' && s[i]<='Z')
    ...
    while (1)
    {
        ...
        for (int i = 0; i < used; i++)
        {
            if (FD_ISSET(nsfd[i],&rfd))
            {
                ...
                if(strcmp(msg.name,"capitalizealt")==0)
                {
                    CapitalizeAlternate(msg.data);
                ...
                else if(strcmp(msg.name,"lowercasealt")==0)
                {
                    lowercaseAlternate(msg.data);
                    write(nsfd[i], &msg, sizeof(msg));
                }
                else
                {
