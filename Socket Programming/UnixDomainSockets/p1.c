...
/* 
	Statement: open a file, read that fd, send that fd using UnixDomainSocket, receive it and read from that file
*/
char *path = "./usoc";
...
int main(int argc, char *argv[]) {
	...
	printf("Client connected with nsfd,%d\n",nufd);
	//int fd1 = open("tmp.txt", O_RDONLY);
	printf("sfd from here is ->%d",usfd);
