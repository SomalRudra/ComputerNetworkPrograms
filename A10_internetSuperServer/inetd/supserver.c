				...
				if(FD_ISSET(sfd[i],&readset))
				{
					if(strcmp(type[i],"tcp")==0)
					{
						...
						else
						{
							...
							{
								close(sfd[i]);
								//both STDIN and STDOUT now replaced by nsfd. hence, when something is typed in client's terminal, the program at execv will read client's terminal's input as own input and output in client's terminal
								dup2(nsfd,0);
								dup2(nsfd,1);
								char* arg[1]= {NULL};
